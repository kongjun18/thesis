...
%中文摘要
\ZhAbstract{
在计算机学科的教学中，工程实践是必不可少的一环。内存分配器负责分配和回收内存，是操作系统内核内存管理子系统的核心组件，被其他所有子系统依赖。内存分配器的设计和实现直接影响系统的整体性能，因此内存分配器在操作系统中具有重要地位。

本论文概述了 Linux 内核物理内存分配器的发展现状，分析了 Linux 内核的内存模型演化进程，完成了基于 RISC-V 指令集的教学操作系统中物理内存管理子系统的设计与实现，包括物理内存探测、页分配器、slab 分配器等。此外，还分析了内存分配器内存碎片情况并提出改进方法。
...
CPU 的 NUMA
架构机器，该节点并不包含整个物理地址空间，仅仅包含系统中的物理内存。节点内的物理内存被进一步划分为多个区域\texttt{struct\ zone}，每个区域都有自己独立的内存管理结构\texttt{struct\ page\ *mem\_map}。上图展示了本论文实现的内存模型的逻辑结构。在实际的实现中，节点存储了整个节点内存区域的\texttt{struct\ page\ mem\_map{[}{]}}，每个\texttt{zone}中的\texttt{struct\ page\ mem\_map{[}{]}}只是指向节点的\texttt{mem\_map{[}{]}}中对应区域的\texttt{struct\ page*}指针。物理结构如图\ref{figure:memory-model-physical-structure}所示。
...
slab 对象的值必须如图\ref{figure:slab-obj-debug-mark}所示，否则发生了内存安全 bug。
