...
\begin{figure}
 \centering
 \includegraphics[scale=0.4]{images/诚信声明.pdf}
\end{figure}

%中文摘要
\ZhAbstract{
在计算机学科的教学中，工程实践是必不可少的一环。内存分配器负责分配和回收内存，是操作系统内核内存管理子系统的核心组件，被其他所有子系统依赖。内存分配器的设计和实现直接影响系统的整体性能，因此在操作系统中具有重要地位。

本论文概述了 Linux 内核物理内存分配器的发展现状，分析了 Linux 内核的内存模型演化进程。论文参考了 Linux 内核和 jemalloc 等著名内存分配器的设计，采用伙伴系统结合 slab 分配器的设计方案，最终完成了一个设计简洁、支持 NUMA(Non-Uniform Memory Access)架构、支持任意大小内存分配并且可用于操作系统教学的内存分配器。此外，还分析了内存分配器内存碎片情况。

论文最终完成了包括物理内存探测、页分配器和slab 分配器和基本的内存安全错误检测的代码实现。相关测试表明，论文设计并实现的内存管理各项功能可以正常运行。
}{操作系统；内存分配器；内存管理；伙伴系统}

%英文摘要
\EnAbstract {In the teaching of computer science, engineering practice is an essential part. The memory allocator is responsible for allocating and reclaiming memory. It is the core component of the memory management subsystem of the operating system kernel and is relied upon by all other subsystems. The design and implementation of the memory allocator directly affects the overall performance of the system, so it plays an important role in the operating system.

This paper summarizes the development status of the Linux kernel physical memory allocator, and analyzes the evolution process of the Linux kernel memory model. The paper refers to the design of famous memory allocators such as Linux kernel and jemalloc, adopts the design scheme of partner system combined with slab allocator, and finally completes a simple design, supports NUMA (Non-Uniform Memory Access) architecture, supports memory allocation of any size and can be used A memory allocator for operating system teaching. In addition, the memory fragmentation of the memory allocator is analyzed.
...
和多个内存控制器，节点通过高速互联网络连接在一起\upcite{drepper2007every}。每个 CPU
...
内存安全 bug
会对内核稳定性产生负面影响。AOSP
安全性文档指出，内存安全 bug 和以原生编程语言处理内存时遇到的错误是 Android 代码库中最常见的问题。此类问题造成了超过 60\% 的高严重程度安全漏洞，并造成了数百万次用户可见的崩溃\upcite{AndroidAndroidOpenSourceProject-2022-09-19}。从第一个 Android
...
语言编写驱动程序\upcite{Linux61LWNnet-2023-05-25}，试图通过引入内存安全型编程语言环境内存安全问题。此外，从
...
\item
  KASAN(The Kernel Address Sanitizer)\upcite{TheKernelAddressSanitizerKASANTheLinuxKerneldocumentation-2023-04-24}：使用 shadow memory
...
\item
  KFENCE(Kernel Electric-Fence)：在 Linux 5.12 版本引入\upcite{KernelElectricFenceKFENCETheLinuxKerneldocumentation-2023-04-24}，仅检测通过
...
内核的内存模型经历了三个阶段：平坦内存模型、非连续内存模型和稀疏内存模型\upcite{MemorytheflatthediscontiguousandthesparseLWNnet-2021-05-21}。
...
\begin{figure}[htb]
\centering
\includesvg[scale=0.6]{images/x86_64-physical-address-space-latex.svg}
\caption{典型的 x86\_64 物理地址空间}
\label{figure:x86_64-physical-address-space}
\end{figure}

x86\_64 架构中，前 1MB
物理地址空间称为实模式地址空间\upcite{DetectingMemoryx86OSDevWiki-2022-09-16}，实模式地址空间布局如表\ref{table:x86-real-mode-address-space}所示。
...
\begin{figure}[htb]
\centering
\includesvg[scale=0.6]{images/FLATMEM.drawio.svg}
...
\begin{figure}[H]
\centering
\includesvg[scale=0.6]{images/DISTCONFIT.drawio.svg}
...
因此，Linux 于 2005 年引入了原始的稀疏内存模型，称为 SPARSEMEM\upcite{sparsememmemorymodelLWNnet-2005-05-06}。SPARSEMEM 模型把粗粒度的节点，变成了更细粒度的\texttt{struct\ mem\_section}。每个\texttt{mem\_seciton}都有自己的内存管理结构\texttt{mem\_map{[}{]}}。一个\texttt{mem\_section}管理\(2^{SECTION\_SIZE\_BIT}\)字节物理内存，通常\texttt{SECTION\_SIZE\_BIT}
...
必须连续存在于整个物理地址的要求，改成了 \texttt{struct\ mem\_section}
...
\begin{figure}[htb]
\centering
\includesvg[scale=0.6]{images/SPARSEMEM.drawio.svg}
...
内存模型，这两个问题已经分别通过 2005 年引入的 SPARSEMEM-EXTREME 拓展\upcite{sparsememextremeimplementationLWNnet-2005-08-12}和
...
\begin{figure}[H]
\centering
\includesvg[width=450pt]{images/SPARSEMEM-EXTREME.drawio.svg}
...
DISCINTIGMEM memory model} 彻底移除了 DISCONFITMEM 内存模型\upcite{RemoveDISCINTIGMEMmemorymodelLWNnet-2021-06-04}，SPARSEMEM
...
\begin{figure}[H]
\centering
\includesvg[scale=0.45]{images/lzuos-memory-model.drawio.svg}
...
\begin{figure}[H]
\centering
\includesvg[scale=0.6]{images/lzuos-memory-model-implementation.drawio.svg}
...
不对物理地址做限制\upcite{russell2008virtio}，但仍然区分了常规内存分配请求和用于 DMA
...
页分配器（page allocator）指按页管理系统内存的分配与释放的内存分配器，是操作系统最基础的组件。本论文的页分配器使用的主要算法是伙伴系统（Binary Buddy Allocator），该算法由 Knowlton \upcite{knowlton1965fast} 提出并由 Knuth \upcite{knuth1968art} 进一步描述。与其他内存分配算法相比，伙伴系统有以下优点：
...
伙伴系统只适合分配\(2^N\)页大小的内存，因此包括 FreeBSD\upcite{dube1998comparison}、Linux 和 jemalloc\upcite{evans2006scalable} 在内的采用伙伴系统算法的内存分配器均只将其用于实现页分配器，然后在其上建立更复杂的内存分配器服务非\(2^N\)大小的内存分配请求。本系统在页分配器上实现 slab 分配器，用于满足非\(2^N\)页大小的内存分配需求。
...
\begin{figure}[H]
\centering
\includesvg[width=450pt]{images/kmem_cache-physical-structure.drawio.svg}
...
\begin{figure}[H]
\centering
\includesvg[width=450pt]{images/on-slab-and-off-slab.drawio.svg}
\caption{slab 管理结构}
\end{figure}
...
\begin{figure}[H]
\centering
\includesvg[scale=0.6]{images/alloc-obj-simple-flowchart.drawio.svg}
...
\begin{figure}[H]
\centering
\includegraphics[width=490pt]{images/alloc-obj-flow-chart.drawio-cropped.pdf}
...
SunOS 5.4 利用 slab 分配器实现了以下功能\upcite{bonwick1994slab}：
...
\begin{figure}[H]
\centering
\includesvg[scale=0.6]{images/slab-debug-layout.drawio.svg}
\caption{slab 对象布局}
...
\begin{figure}[H]
\centering
\includesvg[scale=0.6]{images/slab-debug-status.drawio.svg}
\caption{slab 对象调试标记}
...
分配器的对象缓存策略对无状态内存也同样有效\upcite{bonwick1994slab}，因此\texttt{kmalloc()}只是
...
本文描述的内存分配器虽然完整的实现了内存分配释放功能，但还存在一些缺陷。最主要的是没有实现内存回收（memory reclaim），当空闲内存紧张时，扫描系统内存并回收可回收的内存，如空闲的 slab、磁盘缓冲区等\upcite{bovet2005understanding}。
...
这是论文的结束，是五年大学生涯的终点，也是我职业生涯的起点。Hello, World!

\clearpage
\thispagestyle{empty}
\begin{figure}
 \centering
 \includegraphics[scale=0.4]{images/成绩页.pdf}
\end{figure}
