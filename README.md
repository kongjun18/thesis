...
第一章：绪论。首先阐述了论文选题的背景和意义，随后概述了 Linux 内核内存管理子系统的最新进展，最后介绍本论文的主要工作与组织结构。

第二章：内存模型。首先概述 Linux 内核的内存模型演化，然后介绍本系统使用的内存模型与相关数据结构。

第三章：页分配器。本章描述如何跟踪物理页分配情况，然后介绍页分配器提供的 API 及其实现，最后分析内存碎片情况。

第四章：slab 分配器。本章首先概述 slab 分配器的设计原则，随后介绍 slab 分配器提供的 API 及其实现，最后分析硬件缓存利用情况和内存碎片。

第五章：结论与展望。简要概述系统的总体实现结果，展望未来本操作系统内存分配器的发展。

第六章：附录。附录介绍了其他与实现内存分配器有关的重要主题，包括物理内存探测，建立线性映射区，物理内存分配器的建立，slab 分配器的初始化等。
...
![典型的 x86_64 物理地址空间](images/x86_64-physical-address-space.svg)
...
![SPARSEMEM 内存模型](images/SPARSEMEM.drawio.svg)
...
![本论文实现的内存模型的物理结构](images/lzuos-memory-model-implementation.drawio.svg)
...
![伙伴系统的二叉树实现](images/buddy-system-binary-tree-implementation.jpg)
...
| `ctor`         | `void (ctor)(void _, struct kmem_cache *, uint32_t)` | 对象构造函数                                            |
...
![slab 分配器对象空闲链表分配释放过程](images/alloc-obj-freelist-management.drawio.svg)
