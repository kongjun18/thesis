...
\entitle{{Design of memory allocator for teaching operating system kernel}}
...
第六章：附录。附录介绍了其他与实现内存分配器有关的重要主题，包括物理内存探测，建立线性映射区，物理内存分配器的建立，slab
分配器的初始化等。
...
Buddy Allocator），该算法由 Knowlton {[}Kno65{]} 设计并由 Knuth
{[}Knu68{]} 进一步描述。与其他内存分配算法相比，伙伴系统有以下优点：
...
\begin{figure}
\centering
\includegraphics{images/alloc-obj-flow-chart.drawio.pdf}
